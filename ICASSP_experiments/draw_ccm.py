...
$ python draw_ccm.py\
 --input_dir_n /media/DATA/SOM_NOCS_OUTPUTS/outputs/0300_run_openimagesfixed/inferences_ccm\
 --input_dir_p /media/DATA/SOM_NOCS_OUTPUTS/outputs/0300_run_openimagesfixed/poses_ccm\
 --ccm_dir /media/DATA/SOM_NOCS_DATA/ccm_test_set\
 --output_dir /media/DATA/SOM_NOCS_OUTPUTS/outputs/0300_run_openimagesfixed/drawings_ccm
"""
import argparse
import numpy as np
...
parser.add_argument('--ccm_dir', type=str, help="path to the CCM test set directory")
...
for view in ["view1", "view2", "view3"]:
    ...
    video_indices = os.listdir( os.path.join( args.input_dir_p, view))
    video_indices.sort()
    for video_idx in video_indices:

        # Get the frame predictions
        network_preds = os.listdir( os.path.join(args.input_dir_n, view, video_idx))
        network_preds.sort()
        for pred_file in network_preds:
            ...
            os.makedirs( os.path.join(args.output_dir, view, video_idx), exist_ok=True) 
            ...
            network_pred = np.load( os.path.join(args.input_dir_n, view, video_idx, pred_file), allow_pickle=True).item() # contains predicted masks, coords etc.
            pose_pred = np.load( os.path.join(args.input_dir_p, view, video_idx, pred_file),  allow_pickle=True).item() # contains predicted pose

            # Get RGB image
            rgb_path = os.path.join(args.ccm_dir, "rgb", view, video_idx, "{}.png".format(image_index) )
            ...
            for n in range(0, num_instances):
                ...
                if np.array_equal(pose_pred["umey_PRED_RTs"][n], np.zeros((4,4))):
                    ...
                    pred_RT = pose_pred["epnp_avg_pred_RTs"][n]
                    ...
                    metric_dimensions_mm = pose_pred["epnp_avg_dimensions_mm"][n].transpose()
            ...
            f_rgb_clone = os.path.join(args.output_dir, view, video_idx, "{}_RGB.png".format(image_index))
            cv2.imwrite(f_rgb_clone, rgb_clone[:,:,::-1]) # RGB TO BGR

            f_nocs_out = os.path.join(args.output_dir, view, video_idx, "{}_NOCS.png".format(image_index))
            cv2.imwrite(f_nocs_out, nocs_out[:,:,::-1]) # RGB TO BGR

            f_pose_out = os.path.join(args.output_dir, view, video_idx, "{}_POSE.png".format(image_index))
            cv2.imwrite(f_pose_out, pose_out[:,:,::-1]) # RGB TO BGR

            f_label_out = os.path.join(args.output_dir, view, video_idx, "{}_LABEL.png".format(image_index))
