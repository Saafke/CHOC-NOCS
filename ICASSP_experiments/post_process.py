...
# rot_matrix = cv2.Rodrigues(rvec)[0]; print("rot_matrix:", rot_matrix)
# pred_RT = np.zeros((4,4))
# pred_RT[:3,:3] = rot_matrix
# pred_RT[:3, 3] = np.asarray(tvec).transpose()
# pred_RT[3,3] = 1.0

# Get the average scale factor of this category (millimeters)
# print("cat id:", pred["pred_classes"][n])
# scale_factor = objects_info["categories"][pred["pred_classes"][n]-1]["average_train_scale_factor"]

# pred_RT = np.zeros((4, 4), dtype=np.float32) 

# # Apply the scale to the translation
# tvec *= scale_factor
# pred_R = cv2.Rodrigues(rvec)[0]
# pred_RT[:3, :3] = np.diag([scale_factor, scale_factor, scale_factor]) / 1000 @ pred_R
# pred_RT[:3, 3] = tvec[:,0] / 1000
# pred_RT[3, 3] = 1

# """Get the 8 bounding box points in the NOCS"""
# abs_coord_pts = np.abs(coord_im[mask_im==1] - 0.5)
# print("abs_coord_pts:", abs_coord_pts.shape)
...
# # This pose is for the transforming the METRIC object
# scale_factor = objects_info["categories"][pred["pred_classes"][n]-1]["average_train_scale_factor"]

# pred_RT = np.zeros((4, 4), dtype=np.float32) 
# tvec_ = tvec * scale_factor
# pred_R = cv2.Rodrigues(rvec)[0]
# pred_RT[:3, :3] = pred_R.transpose() # np.diag([scale_factor, scale_factor, scale_factor]) @ pred_R.transpose()
# pred_RT[:3, 3] = tvec_[:,0] # / 1000
# pred_RT[3, 3] = 1
...
# pts, idxs = utils.backproject(depth, intrinsics, mask)
# coord_pts = coord_im[idxs[0], idxs[1], :] - 0.5

# # Show depth points in Open3D
# if False:
# 	# Only Object Depth
# 	pcl = o3d.geometry.PointCloud()
# 	pcl.points = o3d.utility.Vector3dVector(pts)
# 	print(img[idxs])
# 	print(img[idxs].shape)
# 	pcl.colors = o3d.utility.Vector3dVector(img[idxs]/255.0) # color it
...
# # Because we want the pose the transforms the metric object, so, no scaling is required.
# with_scale = False

# scale_factors, rotation, translation, outtransform = estimateSimilarityTransform(coord_pts, pts, False)
# aligned_RT = np.zeros((4, 4), dtype=np.float32) 
# if with_scale:
# 	aligned_RT[:3, :3] = np.diag(scale_factors) / 1000 @ rotation.transpose() # @ = matrix multiplication
# else:
# 	aligned_RT[:3, :3] = rotation.transpose()
# aligned_RT[:3, 3] = translation # NOTE: do we need scaling? #/ 1000 # meters
# aligned_RT[3, 3] = 1
...
# PRED_RTs[n, :, :] = z_180_RT @ aligned_RT 
...
# 	# Add 6D pose and object size to results
# 	umey_PRED_RTs[n, :, :] = umeyama_RT
# 	umey_bbox_scales_in_metric_mm[n, :] = bbox_scales_in_nocs[n, :] * umey_scale_factors
...
# 	umey_bbox_scales_in_metric_mm[n, :] = [0,0,0]
# 	umey_PRED_RTs[n,:,:] = np.zeros([4,4], dtype=np.float32) 
